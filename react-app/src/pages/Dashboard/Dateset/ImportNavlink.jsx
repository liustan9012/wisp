...
import { useUploadNavlink } from "../../../api/data"
...
const ImportNavlink = () => {
  ...
  const { trigger: uploadNavlink, isMutating, data, error } = useUploadNavlink()
    ...
    <Box sx={{ p: 2 }}>
      ...
      <Box sx={{ mt: 2 }}>
        ...
        <Button
          variant="contained"
          disabled={isMutating}
          startIcon={isMutating && <CircularProgress size={24} />}
        ...
        <Dialog open={open} onClose={handleClose} maxWidth={"xs"} fullWidth>
          <DialogTitle id="import-navlink-dialog-title">
            {t("import navlink")}
          </DialogTitle>
          <DialogContent>
            <DialogContentText id="import-navlink-dialog-description">
              {data && data?.msg === "OK"
                ? `success: ${data?.count?.success} failure: ${data?.count?.failure}`
                : ""}
