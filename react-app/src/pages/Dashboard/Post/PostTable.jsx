...
export function PostTableComponet({ order, total, orderBy, page, rowsPerPage, tabledata }) {
    ...
    >
      <Paper variant="outlined" sx={{ minWidth: "70vw", mb: 2 }}>
        <Toolbar sx={{ pl: { sm: 2 },  pr: { xs: 8 }}}>
          <Typography sx={{ pl: 4, flexGrow: 1, textTransform: "capitalize" }}>{t("post")}</Typography>
          <Button
            startIcon={<Add />}
            variant="contained"
            sx={{ flexGrow: 0 }}
        ...
        <TableContainer>
          <Table sx={{ minWidth: 750 }} aria-labelledby="Post Table" size={"small"}>
              ...
              {tabledata.map((post) => {
                return (
                  <TableRow
                    ...
                    sx={{ cursor: "pointer" }}
                  >
                    <TableCell component="th" id={`post-${post.id}`} scope="row" padding="none" align="center">
                      {post.id}
                    </TableCell>
                    <TableCell align="center">
                      <Link underline="none" to={`/admin/post/${post.id}`} component={RouteLink}>
                        {post.title}
                      </Link>
                    </TableCell>
                    <TableCell align="center">
                    ...
                    <TableCell align="center">{t(`${post.status}`.toLowerCase())}</TableCell>
                    <TableCell align="center">
                      <Button
                        variant="outlined"
                        size="small"
